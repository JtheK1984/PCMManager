...
uses
  System.Types, System.Classes, Winapi.Windows, System.Contnrs, System.Generics.Collections,
  dxCore,
  cxDateUtils, cxSchedulerStorage, cxSchedulerCustomControls, cxSchedulerUtils,
  cxVariants, cxSchedulerRecurrence;
...
type
  ...
  protected
    ...
    function GetStandardDate: TDateTime;
  ...
  TcxSchedulerICalendarEventSequenceProperty = class(TcxSchedulerICalendarEventCustomProperty)
  protected
    class function TokenName: string; override;
    function GetValue: Variant; override;
  end;

  TcxSchedulerICalendarEventExDateProperty = class(TcxSchedulerICalendarEventDateTimeProperty)
  protected
    function CanExport: Boolean; override;
    class function TokenName: string; override;
    function GetValue: Variant; override;
  ...
  TcxSchedulerICalendarEventRecurrenceIDProperty = class(TcxSchedulerICalendarEventDateTimeProperty)
  protected
    function CanExport: Boolean; override;
    class function TokenName: string; override;
    function GetValue: Variant; override;
  ...
  TcxSchedulerICalendarEventBusyStatusProperty = class(TcxSchedulerICalendarEventCustomProperty)
  protected
    class function TokenName: string; override;
    function GetValue: Variant; override;
    procedure SetValue(const AValue: Variant); override;
  ...
  TcxSchedulerICalendarEventStatus = (esConfirmed, esCancelled, esTentative);
  ...
  private
    FExceptions: TList<TDateTime>;
    FRecurrenceDate: TDate;
    FStatus: TcxSchedulerICalendarEventStatus;
    FUID: Variant;
    function GetEvent: TcxSchedulerEvent;
  ...
  protected
    ...
    procedure AfterImport(AStrings: TStringList); override;
    procedure BeforeImport(AStrings: TStringList); override;
    procedure BeforeExport(AStrings: TStringList); override;
    function CanExport: Boolean; override;
    ...
    class function GetRecurrenceDate(APattern: TcxSchedulerEvent; ARecurrenceIndex: Integer): TDateTime; static;

    property Event: TcxSchedulerEvent read GetEvent;
    property Exceptions: TList<TDateTime> read FExceptions;
    ...
    property RecurrenceDate: TDate read FRecurrenceDate write FRecurrenceDate;
    property Status: TcxSchedulerICalendarEventStatus read FStatus write FStatus;
  ...
  private
    FDateTime: TDateTime;
    FPatternIDs: TdxVariantDictionary<string>;
    FPatterns: TdxVariantDictionary<TcxSchedulerEvent>;
    FTimeZone: Integer;
  ...
  protected
    ...
    property PatternIDs: TdxVariantDictionary<string> read FPatternIDs;
    property Patterns: TdxVariantDictionary<TcxSchedulerEvent> read FPatterns;
    property TimeZone: Integer read FTimeZone write FTimeZone;
  public
...
uses
  System.SysUtils, Vcl.Forms, System.StrUtils, System.Variants, System.DateUtils, System.Math, cxClasses;

const
  dxThisUnitName = 'cxSchedulerICalendar';
...
begin
  ...
  Result := (ADateTimeProperty.TimeZone <> ADateTimeProperty.ICalendar.TimeZone) and
    (ADateTimeProperty.TimeZone <> -1);
  if Result then
...
function TcxSchedulerICalendarTimeZoneObject.GetStandardDate: TDateTime;
begin
...
begin
  Result := GetStandardDate = GetDaylightDate;
end;
...
procedure TcxSchedulerICalendarEventUIDProperty.SetValue(const AValue: Variant);
begin
  inherited SetValue(AValue);
  Parent.UID := AValue;
end;
...
var
  AResult: string;
begin
  if (Event.EventType in [etException, etCustom]) and ICalendar.PatternIDs.TryGetValue(Event.Pattern.ID, AResult) then
    Result := AResult
  else
  begin
    AResult := dxGenerateGUID;
    Delete(AResult, 1, 1);
    Delete(AResult, Length(AResult), 1);
    if Event.EventType = etPattern then
      ICalendar.PatternIDs.AddOrSetValue(Event.ID, AResult);
    Result := AResult;
  end;
end;

{ TcxSchedulerICalendarEventSequenceProperty }

class function TcxSchedulerICalendarEventSequenceProperty.TokenName: string;
begin
  Result := 'SEQUENCE';
end;

function TcxSchedulerICalendarEventSequenceProperty.GetValue: Variant;
begin
  case Event.EventType of
    etPattern: Result := 1;
    etCustom, etException: Result := 2;
  else
    Result := 0;
  end;
end;

{ TcxSchedulerICalendarEventExDateProperty }

class function TcxSchedulerICalendarEventExDateProperty.TokenName: string;
begin
  Result := 'EXDATE';
end;

function TcxSchedulerICalendarEventExDateProperty.CanExport: Boolean;
begin
  Result := Parent.Exceptions.Count > 0;
end;

procedure TcxSchedulerICalendarEventExDateProperty.DoExport(
  AStrings: TStringList);
begin
  while Parent.Exceptions.Count > 0 do
  begin
    inherited DoExport(AStrings);
    Parent.Exceptions.Delete(0);
  end;
end;

function TcxSchedulerICalendarEventExDateProperty.GetValue: Variant;
begin
  Result := Parent.Exceptions[0];
end;

procedure TcxSchedulerICalendarEventExDateProperty.SetValue(const AValue: Variant);
begin
  Parent.Exceptions.Add(AValue);
end;

{ TcxSchedulerICalendarEventRecurrenceIDProperty }

class function TcxSchedulerICalendarEventRecurrenceIDProperty.TokenName: string;
begin
  Result := 'RECURRENCE-ID';
end;

function TcxSchedulerICalendarEventRecurrenceIDProperty.CanExport: Boolean;
begin
  Result := (Event.EventType = etCustom) and inherited CanExport;
end;

function TcxSchedulerICalendarEventRecurrenceIDProperty.GetValue: Variant;
begin
  Result := Parent.GetRecurrenceDate(Event.Pattern, Event.RecurrenceIndex);
  TimeZone := -1;
end;

procedure TcxSchedulerICalendarEventRecurrenceIDProperty.SetValue(const AValue: Variant);
begin
  Parent.RecurrenceDate := AValue;
end;
...
begin
  Result := inherited CanExport and (Event.EventType = etPattern);
end;
...
class function TcxSchedulerICalendarEventBusyStatusProperty.TokenName: string;
begin
...
function TcxSchedulerICalendarEventBusyStatusProperty.GetValue: Variant;
begin
  Result := StatusMap[Event.State];
end;

procedure TcxSchedulerICalendarEventBusyStatusProperty.SetValue(const AValue: Variant);
var
  AStatus: Integer;
  I: Integer;
begin
...
const
  AResultMap: array[Boolean] of string = ('CONFIRMED', 'CANCELLED');
begin
  Result := AResultMap[Event.EventType = etException];
end;

procedure TcxSchedulerICalendarEventStatusProperty.SetValue(const AValue: Variant);
begin
  if CompareText(AValue, 'CANCELLED') = 0 then
    Parent.Status := esCancelled
  else if CompareText(AValue, 'TENTATIVE') = 0 then
    Parent.Status := esTentative
  else
    Parent.Status := esConfirmed;
end;
...
constructor TcxSchedulerICalendarEventObject.Create(
  AICalendar: TcxSchedulerICalendar;
  AParent: TcxSchedulerICalendarCustomObject);
begin
  inherited Create(AICalendar, AParent);
  FExceptions := TList<TDateTime>.Create;
end;

destructor TcxSchedulerICalendarEventObject.Destroy;
begin
...
procedure TcxSchedulerICalendarEventObject.AfterImport(AStrings: TStringList);
  ...
  function GetRecurrenceIndex(APattern: TcxSchedulerEvent; ADate: TDateTime): Integer;
  begin
    Result := -1;
    ADate := dxDateOf(ADate);
    with TcxSchedulerEventRecurrenceInfoAccess(APattern.RecurrenceInfo).GetCalculatorClass.Create(APattern, ADate, ADate + 1) do
    try
...
begin
  ...
  if (RecurrenceDate <> InvalidDate) and not VarIsNull(UID) and ICalendar.Patterns.TryGetValue(UID, APattern) then
  begin
    Event.ParentID := APattern.ID;
    if Status = esConfirmed then
      Event.EventType := etCustom
    else
      Event.EventType := etException;
    Event.RecurrenceIndex := GetRecurrenceIndex(APattern, RecurrenceDate);
  end;
  Event.Post;
  if Event.EventType = etPattern then
  begin
    if not VarIsNull(UID) then
      ICalendar.Patterns.Add(UID, Event);
    while Exceptions.Count > 0 do
    begin
      with Event.Storage.createEvent do
      begin
        ParentID := Event.ID;
        EventType := etException;
        RecurrenceIndex := GetRecurrenceIndex(Event, Exceptions[0]);
        Post;
      end;
      Exceptions.Delete(0);
    end;
  end;
  inherited AfterImport(AStrings);
end;

procedure TcxSchedulerICalendarEventObject.BeforeExport(AStrings: TStringList);
var
  AEvent: TcxSchedulerEvent;
begin
  inherited BeforeExport(AStrings);
  Exceptions.Clear;
  if not Event.HasExceptions then
    Exit;
  AEvent := TcxSchedulerEventAccess(Event).Link;
  while AEvent <> nil do
  begin
    if AEvent.EventType = etException then
      FExceptions.Add(GetRecurrenceDate(Event, AEvent.RecurrenceIndex));
    AEvent := TcxSchedulerEventAccess(AEvent).Link;
  end;
end;

procedure TcxSchedulerICalendarEventObject.BeforeImport(AStrings: TStringList);
begin
  inherited BeforeImport(AStrings);
  FUID := NULL;
  RecurrenceDate := InvalidDate;
  Status := esConfirmed;
  Exceptions.Clear;
end;

function TcxSchedulerICalendarEventObject.CanExport: Boolean;
begin
  Result := inherited CanExport and (Event.EventType in [etNone, etPattern, etCustom]);
end;
...
begin
  ...
  AddProperty(TcxSchedulerICalendarEventBusyStatusProperty);
  ...
  AddProperty(TcxSchedulerICalendarEventRecurrenceIDProperty);
  AddProperty(TcxSchedulerICalendarEventStatusProperty);
  AddProperty(TcxSchedulerICalendarEventSequenceProperty);
  AddProperty(TcxSchedulerICalendarEventExDateProperty);
end;

class function TcxSchedulerICalendarEventObject.GetRecurrenceDate(APattern: TcxSchedulerEvent; ARecurrenceIndex: Integer): TDateTime;
begin
  if ARecurrenceIndex > -1 then
  begin
    with TcxSchedulerEventRecurrenceInfoAccess(APattern.RecurrenceInfo).GetCalculatorClass.Create(APattern, APattern.RecurrenceInfo.Start, APattern.RecurrenceInfo.Finish) do
    try
      CalcOccurrence(ARecurrenceIndex + 1);
...
begin
  ...
  FPatterns := TdxVariantDictionary<TcxSchedulerEvent>.Create;
  FPatternIDs := TdxVariantDictionary<string>.Create;
end;

destructor TcxSchedulerICalendar.Destroy;
begin
  FreeAndNil(FPatternIDs);
...
begin
  ...
  TcxSchedulerICalendarEventSequenceProperty.Register;
  TcxSchedulerICalendarEventExDateProperty.Register;
  ...
  TcxSchedulerICalendarEventBusyStatusProperty.Register;
  TcxSchedulerICalendarEventRecurrenceProperty.Register;
  TcxSchedulerICalendarEventUIDProperty.Register;
  TcxSchedulerICalendarEventRecurrenceIDProperty.Register;
...
begin
  ...
  TcxSchedulerICalendarEventBusyStatusProperty.Unregister;
  TcxSchedulerICalendarEventRecurrenceProperty.Unregister;
  TcxSchedulerICalendarEventUIDProperty.Unregister;
  TcxSchedulerICalendarEventRecurrenceIDProperty.Unregister;
  TcxSchedulerICalendarEventStatusProperty.Unregister;
  TcxSchedulerICalendarEventSequenceProperty.Unregister;
  TcxSchedulerICalendarEventExDateProperty.Unregister;

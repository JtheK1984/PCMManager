...
uses
  System.Types, Winapi.Windows, System.Classes, System.SysUtils, Vcl.Graphics, Data.DB, System.RTLConsts, dxCoreClasses, cxDB, cxDBData, cxSchedulerStorage,
...
type
  ...
  private
    ...
    function GetStorage: TcxSchedulerDBStorage; inline;
  protected
  ...
  private
    ...
    FResourceVisibleField: TcxDBDataField;
    FResourceVisibleFieldName: TcxFieldName;
    FResourceWorkDaysField: TcxDBDataField;
    FResourceWorkDaysFieldName: TcxFieldName;
    FResourceWorkFinishField: TcxDBDataField;
    FResourceWorkFinishFieldName: TcxFieldName;
    FResourceWorkStartField: TcxDBDataField;
    FResourceWorkStartFieldName: TcxFieldName;
    FOnResourceLoaded: TcxSchedulerResourceEvent;
    ...
    procedure SetResourceVisibleFieldName(const AValue: TcxFieldName);
    procedure SetResourceWorkDaysFieldName(const AValue: TcxFieldName);
    procedure SetResourceWorkFinishFieldName(const AValue: TcxFieldName);
    procedure SetResourceWorkStartFieldName(const AValue: TcxFieldName);
  protected
    function CreateDataController: TcxSchedulerDBStorageResourceDataController; virtual;
    procedure DoResourceLoaded(AResource: TcxSchedulerStorageResourceItem);
    ...
    property ResourceVisibleField: TcxDBDataField read FResourceVisibleField;
    property ResourceWorkDaysField: TcxDBDataField read FResourceWorkDaysField;
    property ResourceWorkFinishField: TcxDBDataField read FResourceWorkFinishField;
    property ResourceWorkStartField: TcxDBDataField read FResourceWorkStartField;
  public
  ...
  published
    ...
    property ResourceVisible: TcxFieldName read FResourceVisibleFieldName write SetResourceVisibleFieldName;
    property ResourceWorkDays: TcxFieldName read FResourceWorkDaysFieldName write SetResourceWorkDaysFieldName;
    property ResourceWorkFinish: TcxFieldName read FResourceWorkFinishFieldName write SetResourceWorkFinishFieldName;
    property ResourceWorkStart: TcxFieldName read FResourceWorkStartFieldName write SetResourceWorkStartFieldName;
    property Images;
    property Items;
    property OnGetResourceImageIndex;
    property OnGetResourceName;
    property OnResourceLoaded: TcxSchedulerResourceEvent read FOnResourceLoaded write FOnResourceLoaded;
  end;
...
uses
  System.Variants, cxDateUtils, dxCore, cxVariants, cxSchedulerStrs, cxSchedulerUtils;


type
  TcxSchedulerEventAccess = class(TcxSchedulerEvent);
  TcxCustomDataFieldAccess = class(TcxCustomDataField);
  TcxSchedulerStorageResourceItemAccess = class(TcxSchedulerStorageResourceItem);
...
var
  ...
  AStorage: TcxSchedulerDBStorage;
...
begin
  ...
  AStorage := Storage;
  for I := 0 to AStorage.FieldCount - 1 do
  begin
    AStorageField := TcxSchedulerDBStorageField(AStorage.Fields[I]);
    AValueDef := AStorageField.ValueDef;
    ATextStored := AValueDef.TextStored;
    AField := AStorageField.Field;
    if AField <> nil then
    begin
      TcxDataStorageHelper.SetValue(AHandle, AValueDef, GetFieldValue(AField));
      if ATextStored then
         TcxDataStorageHelper.SetDisplayText(AHandle, AValueDef, AField.DisplayText);
    end
    else
    begin
...
begin
  ...
  FreeAndNil(FResourceVisibleField);
  FreeAndNil(FResourceWorkDaysField);
  FreeAndNil(FResourceWorkFinishField);
  FreeAndNil(FResourceWorkStartField);
...
begin
  ...
  begin
    ...
    ResourceVisible := TcxSchedulerDBStorageResources(Source).ResourceVisible;
    ResourceWorkDays := TcxSchedulerDBStorageResources(Source).ResourceWorkDays;
    ResourceWorkFinish := TcxSchedulerDBStorageResources(Source).ResourceWorkFinish;
    ResourceWorkStart := TcxSchedulerDBStorageResources(Source).ResourceWorkStart;
  end;
end;
...
procedure TcxSchedulerDBStorageResources.DoResourceLoaded(AResource: TcxSchedulerStorageResourceItem);
begin
  if Assigned(FOnResourceLoaded) then
    FOnResourceLoaded(Storage, AResource);
end;
...
begin
  with TcxSchedulerStorageResourceItemAccess(AItem) do
  begin
    FResourceID := GetValueDef(AIndex, ResourceIDField, Null);
    FImageIndex := GetValueDef(AIndex, ResourceImageIndexField, -1);
    FName := GetValueDef(AIndex, ResourceNameField, '');
    FColor := GetValueDef(AIndex, ResourceColorField, clDefault);
    FVisible := GetValueDef(AIndex, ResourceVisibleField, True);
    FWorkDays := cxIntegerToDays(GetValueDef(AIndex, ResourceWorkDaysField, cxDaysToInteger(DateTimeHelper.WorkDays)));
    FWorkFinish := GetValueDef(AIndex, ResourceWorkFinishField, DateTimeHelper.WorkFinish);
    FWorkStart := GetValueDef(AIndex, ResourceWorkStartField, DateTimeHelper.WorkStart);
  end;
end;
    ...
    try
      ...
      begin
        ...
        for J := 0 to APrevItems.Count - 1 do
          if VarEquals(AItem.ResourceID, APrevItems[J].ResourceID) then
          begin
            with TcxSchedulerStorageResourceItemAccess(AItem) do
            begin
              Assign(APrevItems[J]);
              FResourceID := GetValueDef(I, ResourceIDField, ResourceID);
              FImageIndex := GetValueDef(I, ResourceImageIndexField, -1);
              FName := GetValueDef(I, ResourceNameField, Name);
              FColor := GetValueDef(I, ResourceColorField, Color);
              FVisible := GetValueDef(I, ResourceVisibleField, Visible);
              FWorkDays := cxIntegerToDays(GetValueDef(I, ResourceWorkDaysField, cxDaysToInteger(WorkDays)));
              FWorkFinish := GetValueDef(I, ResourceWorkFinishField, WorkFinish);
              FWorkStart := GetValueDef(I, ResourceWorkStartField, WorkStart);
            end;
            Break;
          end;
        DoResourceLoaded(AItem);
      end;
    finally
      APrevItems.Free;
    end;
...
procedure TcxSchedulerDBStorageResources.SetResourceVisibleFieldName(const AValue: TcxFieldName);
begin
  SetInternalFieldName(FResourceVisibleFieldName, AValue, FResourceVisibleField);
end;

procedure TcxSchedulerDBStorageResources.SetResourceWorkDaysFieldName(const AValue: TcxFieldName);
begin
  SetInternalFieldName(FResourceWorkDaysFieldName, AValue, FResourceWorkDaysField);
end;

procedure TcxSchedulerDBStorageResources.SetResourceWorkFinishFieldName(const AValue: TcxFieldName);
begin
  SetInternalFieldName(FResourceWorkFinishFieldName, AValue, FResourceWorkFinishField);
end;

procedure TcxSchedulerDBStorageResources.SetResourceWorkStartFieldName(const AValue: TcxFieldName);
begin
  SetInternalFieldName(FResourceWorkStartFieldName, AValue, FResourceWorkStartField);
end;
